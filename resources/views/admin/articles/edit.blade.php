    ...
    <div class="bg-white rounded-xl shadow-lg p-8">
        ...
        <div>
            ...
            <input type="text" id="slug" name="slug" value="{{ old('slug', $article->slug) }}"
                   ...
                   placeholder="Akan di-generate otomatis dari judul artikel">
            <p class="mt-1 text-sm text-gray-500">Slug akan digunakan di URL. Kosongkan untuk auto-generate dari judul artikel.</p>
            @error('slug')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea id="excerpt" name="excerpt" rows="3"
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary @error('excerpt') border-red-500 @enderror">{{ old('excerpt', $article->excerpt) }}</textarea>
        ...
        <div>
            ...
            <textarea id="meta_description" name="meta_description" rows="3" maxlength="500"
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary @error('meta_description') border-red-500 @enderror"
                      placeholder="Deskripsi singkat untuk SEO (maksimal 500 karakter)">{{ old('meta_description', $article->meta_description) }}</textarea>
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="20" required
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary @error('content') border-red-500 @enderror">{{ old('content', $article->content) }}</textarea>
            @error('content')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Format yang didukung: JPEG, PNG, JPG, GIF. Maksimal 2MB. Biarkan kosong jika tidak ingin mengubah gambar.</p>
