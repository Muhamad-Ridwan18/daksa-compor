...
<body class="font-sans antialiased bg-background">
    ...
    @php
        ...
        $waUrl = $waNumber ? ('https://wa.me/' . $waNumber . ($waText !== '' ? ('?text=' . urlencode($waText)) : '')) : '#contact';
    @endphp
    <nav class="bg-white sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                <div class="flex items-center">
                    <a href="{{ route('home') }}" class="flex-shrink-0 flex items-center gap-2 group">
                        @if(isset($settings['logo']) && $settings['logo'])
                            <img class="h-10 w-auto transition-transform duration-300 group-hover:scale-105" 
                                 src="{{ Storage::url($settings['logo']) }}" 
                                 alt="{{ $settings['company_name'] ?? 'Logo' }}">
                        @else
                            <div class="flex items-center gap-2">
                                <div class="w-9 h-9 bg-gradient-to-br from-primary to-secondary rounded-lg flex items-center justify-center shadow-md group-hover:shadow-lg transition-all duration-300">
                                    <span class="text-white font-bold text-lg">{{ substr($settings['company_name'] ?? 'D', 0, 1) }}</span>
                                </div>
                                <span class="text-xl font-bold text-primary group-hover:text-secondary transition-colors duration-300">
                ...
                <!-- Desktop Menu -->
                <div class="hidden md:flex items-center space-x-0 flex-nowrap">
                    <a href="{{ route('home') }}" 
                       class="text-gray-700 hover:text-primary hover:bg-primary/5 px-2 py-2 rounded-lg text-sm font-medium transition-all duration-300 whitespace-nowrap {{ request()->routeIs('home') ? 'text-primary bg-primary/10' : '' }}">
                        Beranda
                    </a>
                    <a href="{{ route('about') }}" 
                       class="text-gray-700 hover:text-primary hover:bg-primary/5 px-2 py-2 rounded-lg text-sm font-medium transition-all duration-300 whitespace-nowrap {{ request()->routeIs('about') ? 'text-primary bg-primary/10' : '' }}">
                        Tentang
                    </a>
                    <a href="{{ route('home') }}#services" 
                       class="text-gray-700 hover:text-primary hover:bg-primary/5 px-2 py-2 rounded-lg text-sm font-medium transition-all duration-300 whitespace-nowrap">
                        Layanan
                    </a>
                    ...
                    <a href="{{ route('blog.index') }}" 
                       class="text-gray-700 hover:text-primary hover:bg-primary/5 px-2 py-2 rounded-lg text-sm font-medium transition-all duration-300 whitespace-nowrap {{ request()->routeIs('blog.*') ? 'text-primary bg-primary/10' : '' }}">
                        Blog
                    </a>
                    ...
                    <div class="relative group flex-shrink-0">
                        <button
                            type="button"
                            class="inline-flex items-center text-gray-700 hover:text-primary hover:bg-primary/5 px-2 py-2 rounded-lg text-sm font-medium transition-all duration-300 whitespace-nowrap {{ request()->routeIs('pph21-calculator.*') || request()->routeIs('pph21-masa-terakhir.*') || request()->routeIs('pph-badan-calculator.*') ? 'text-primary bg-primary/10' : '' }}"
                        >
                            Kalkulator Pajak
                            <svg class="ms-1 h-4 w-4 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                    ...
                    <a href="{{ route('careers.index') }}" 
                       class="text-gray-700 hover:text-primary hover:bg-primary/5 px-2 py-2 rounded-lg text-sm font-medium transition-all duration-300 whitespace-nowrap {{ request()->routeIs('careers.*') ? 'text-primary bg-primary/10' : '' }}">
                        Karier
                    </a>
                    <a href="{{ $waUrl }}" 
                       @if($waNumber) target="_blank" rel="noopener" @endif
                       class="bg-primary text-white hover:bg-opacity-90 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-300 hover:shadow-lg ml-1 flex items-center gap-2 whitespace-nowrap flex-shrink-0">
        ...
        <div id="mobile-menu" class="hidden md:hidden bg-white border-t border-gray-100" role="menu" aria-label="Mobile navigation menu">
            <div class="px-4 pt-2 pb-4 space-y-1">
                ...
                <a href="{{ route('about') }}" 
                   class="block text-gray-700 hover:text-primary hover:bg-primary/5 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-300 {{ request()->routeIs('about') ? 'text-primary bg-primary/10' : '' }}">
                    Tentang
                </a>
                <a href="{{ route('home') }}#services" 
    ...
    <script>
        ...
        <!-- Google tag (gtag.js) -->
        <script async src="https://www.googletagmanager.com/gtag/js?id=AW-17796193730"></script>
        <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());

        gtag('config', 'AW-17796193730');
    </script>
    @stack('scripts')
</body>
</html>
