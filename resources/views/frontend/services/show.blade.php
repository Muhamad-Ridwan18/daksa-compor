            ...
            <div class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-primary/20 transform hover:-translate-y-2 flex flex-col h-full">
                ...
                <div class="p-6 pt-0 mt-auto">
                    ...
                    <div class="flex flex-row gap-3 w-full">
                        <button onclick="openOrderModal('{{ $service->id }}', '{{ $product->id }}', '{{ $product->name }}')" 
                                class="flex-1 py-4 px-6 bg-primary text-white rounded-xl font-bold text-lg hover:bg-primary/90 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
                            Pilih Paket Ini
                        </button>
                        @if($waNumber)
                        <a href="{{ $waUrl }}" 
                           target="_blank" 
                           rel="noopener"
                           class="flex items-center justify-center py-4 px-4 text-white rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
                           style="background-color: #25D366; min-width:56px; min-height:56px;"
